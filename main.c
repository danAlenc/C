...
void cadastros(){														//Função para cadastros de cliente e colaboradores
    ...
    if(op == 1){ 														// Valida a opção selecionada
        ...
        retorno();									                    //chama função de conagem regressiva para menu anterior
    ...
    if(op > 2 || op < 0){                                               //Tratamento para o caso do cliente digitar opção invalida
...
void relatorios(){														//Função para relatorios serem executados
    ...
    int op;                                                             //Variavel das opções do usuario
    int i;                                                              //contador
    ...
    printf("0) Sair\n");
    scanf("%d", &op);
    if(op == 1){                                                        //validação da opção do usuario
        system("cls");
        FILE *rendm;
        rendm = fopen("historyDF.db", "r");                             //Abre o arquivo em modo leitura
        if(rendm == NULL){                                              //Validação do arquivo
            printf("Erro no arquivo [ir]\n");
            system("pause");
            retorno();
            return(relatorios());
        ...
        printf("----------------------------------------\n");
        while(fgets(arqir, 100, rendm)!= NULL){;                        //Laço para que, enquanto não retorne vazio, continue lendo
    ...
    if(op == 2){                                                        //validação da opção do usuario
        system("cls");
        FILE *rendm;
    ...
    if(op == 3){                                                        //Validação da opção do usuario
        system("cls");
        FILE *rendm;
    ...
    if(op == 4){                                                        //validação da opção do usuario
        system("cls");
        FILE *rendm;
    ...
    if(op > 4 || op < 0){                                               //Tratamento para o caso do cliente digitar opção invalida
        printf("Opção invalida! Verifique!\n");
        system("pause");
        return(relatorios());
    }
...
void inicio(){															//Menu principal
    ...
    int op;                                                             //Variavel que armazena a opção do usuario
    int i;                                                              //Contador
    while(op != 4){														//Tela de bem vindo em laço até o usuario optar por sair do sistema
        ...
        scanf("%d", &op);
        if(op == 1){ 	    											//Validação da opção do usuario
            cadastros();                                                //chama função de cadastros
        }
        if(op == 2){ 													//validação da opção do usuario
            financeiro();                                               //chama função do financeiro
        }
        if(op == 3){ 													//Validação da opção do usuario
            relatorios();                                               //chama função de relatorios
        }
        if(op == 4){ 													//Validação do usuario
        ...
        if(op > 4 || op < 1){ 											//Tratamento para o caso do cliente digitar opção invalida
